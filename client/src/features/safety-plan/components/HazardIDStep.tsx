...
import { AlertTriangle, Pencil, ClipboardCheck, FileText, GraduationCap } from "lucide-react";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import { cn } from "@/lib/utils";

function getRiskColor(score: number) {
  if (score >= 12) return { bg: "bg-red-500", text: "text-white", label: "Extreme" };
  if (score >= 8) return { bg: "bg-orange-500", text: "text-white", label: "High" };
  if (score >= 4) return { bg: "bg-amber-400", text: "text-amber-900", label: "Medium" };
