...
export function StepperBar({ currentStep, completedSteps, onStepClick, prefilled, taskSummary, onEditDetails, onExit }: StepperBarProps) {
  return (
    <>
      {/* ── Mobile: horizontal stepper pills ── */}
      <div className="md:hidden flex items-center gap-1 px-2 py-2.5 border-b border-border bg-muted/30 overflow-x-auto">
        {STEPS.map((step, idx) => {
          ...
          return (
            <div key={step.n} className="flex items-center gap-1 shrink-0">
              {idx > 0 && (
                <div className={cn("w-3 h-px", done ? "bg-primary" : "bg-border")} />
              )}
              <button
                type="button"
                disabled={!clickable}
                ...
                className={cn(
                  "flex items-center gap-1 rounded-full text-[11px] font-medium transition-colors whitespace-nowrap",
                  active ? "px-2.5 py-1.5" : "px-1.5 py-1.5",
                  active
              ...
              >
                <span className={cn(
                  ...
                  !done && !active && "border border-muted-foreground/20 text-muted-foreground/30"
                )}>
        ...
        {onExit && (
          <button
            type="button"
            onClick={onExit}
            className="text-muted-foreground/40 hover:text-destructive transition-colors shrink-0 p-1.5"
          >
